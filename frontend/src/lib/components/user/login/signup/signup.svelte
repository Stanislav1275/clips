...
<form use:form method="POST" action="/SignUp" class="flex flex-col items-center p-5 mt-6">
    ...
    <div class="flex flex-col-reverse w-full">
        <div class="flex flex-row mb-2 w-4/5 self-center">
            <input name="pass-confirm" type={show ? "text" : "password"} placeholder="Повторите пароль" use:validators={[passwordMatch, required]} data-suf-ignore autocomplete="off" class="rounded-xl p-2 my-2 w-full self-center bg-slate-600 text-gray-200 focus:bg-gray-200 focus:text-slate-700 transition-all duration-300">
        </div>
        <Hint for=pass-confirm on="passwordMatch" class="mt-6 mb-2 text-gray-200 self-center">Пароли должны совпадать</Hint>
    </div>

    <button disabled={!$form.valid} type="submit" class="rounded-2xl p-3 bg-orange-600 text-gray-200 hover:bg-orange-100 mt-5 mb-2 disabled:bg-slate-600 transition-all duration-300">Зарегистрироваться</button>
</form>